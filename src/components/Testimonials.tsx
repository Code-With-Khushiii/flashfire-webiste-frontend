...
/**
 * Auto-scrolling (marquee) testimonials wall + your existing video cards.
 * - Images: infinitely scroll up in 4 columns with different speeds.
 * - Hover/touch: pauses the animation.
 * - Reduced motion: respects prefers-reduced-motion.
 *
 * Drop-in replacement for your current TestimonialsGrid.
 */

const customStyles = `
  /* --- existing animation utilities can stay in your globals --- */
  ...
  .marquee-col {
    ...
    /* Allow hover to pause via parent group */
    animation-play-state: var(--state, running);
  }

  /* Pause on hover for pointer devices */
  .marquee-group:hover .marquee-col { animation-play-state: paused; }
...
// You can keep using your real images here
const screenshots = [
  "/images/image1.jpg",
  "/images/image3.jpg",
  "/images/image4.jpg",
...
const rotations = ["rotate-2", "-rotate-2", "rotate-1", "-rotate-1", "rotate-3", "-rotate-3", "rotate-1", "-rotate-2", "rotate-2", "-rotate-1"]
    ...
    <div className={`mb-2 inline-block w-full rounded-xl shadow-md transform ${rotations[index % rotations.length]} hover:rotate-0 transition-all duration-300 p-2`}>
      <div className="relative overflow-hidden">
        ...
        {testimonial.linkedinUrl !== undefined && (
          <a
            href={testimonial.linkedinUrl || undefined}
            target={testimonial.linkedinUrl ? "_blank" : undefined}
            rel="noreferrer"
...
export default function TestimonialsGrid() {
  ...
  const speeds = [42, 48, 54, 46] // seconds (adjust per your video vibe)

  return (
    <section id="testimonials" className="scroll-mt-28 bg-gradient-to-br from-orange-400 via-orange-500 to-red-500 min-h-screen py-16 px-6 rounded-[3rem] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {/* IMAGE TESTIMONIALS — Infinite vertical marquee */}
          ...
          {cols.map((colItems, colIdx) => {
            ...
            return (
              <div key={colIdx} className="relative overflow-hidden rounded-xl">
                {/* The moving inner track; we duplicate the content so the loop is seamless */}
                <div className="marquee-col" style={{ ['--dur' as any]: dur }}>
                  <div className="flex flex-col gap-3 pb-3">
                    {colItems.concat(colItems).map((src, i) => (
                      <div
                        key={i}
                        className={`rounded-xl shadow-md transform ${rotations[i % rotations.length]} hover:rotate-0 transition-all duration-300 p-2 bg-white/5 backdrop-blur-[1px]`}
                      >
                        <img src={src} alt={`Testimonial ${colIdx}-${i}`} className="rounded-lg w-full h-auto object-contain" />
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* VIDEO TESTIMONIALS (kept, not auto-scrolling to avoid motion overload) */}
